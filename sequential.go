...
func (r *ram8) read(address []bool) []bool {
	return mux8WayMulti(r.registers[0].out, r.registers[1].out, r.registers[2].out, r.registers[3].out, r.registers[4].out, r.registers[5].out, r.registers[6].out, r.registers[7].out, address)
}
...
func (r *ram64) read(address []bool) []bool {
	return mux8WayMulti(r.ram8s[0].read(address[:3]), r.ram8s[1].read(address[:3]), r.ram8s[2].read(address[:3]), r.ram8s[3].read(address[:3]), r.ram8s[4].read(address[:3]), r.ram8s[5].read(address[:3]), r.ram8s[6].read(address[:3]), r.ram8s[7].read(address[:3]), address[3:])
}
...
func (r *ram512) tick(in []bool, address []bool, load bool) {
	a0, a1, a2, a3, a4, a5, a6, a7 := dmux8Way(load, address[6:])
	r.ram64s[0].tick(in, address[:6], a0)
	r.ram64s[1].tick(in, address[:6], a1)
	r.ram64s[2].tick(in, address[:6], a2)
	r.ram64s[3].tick(in, address[:6], a3)
	r.ram64s[4].tick(in, address[:6], a4)
	r.ram64s[5].tick(in, address[:6], a5)
	r.ram64s[6].tick(in, address[:6], a6)
	r.ram64s[7].tick(in, address[:6], a7)
}

func (r *ram512) read(address []bool) []bool {
	return mux8WayMulti(r.ram64s[0].read(address[:6]), r.ram64s[1].read(address[:6]), r.ram64s[2].read(address[:6]), r.ram64s[3].read(address[:6]), r.ram64s[4].read(address[:6]), r.ram64s[5].read(address[:6]), r.ram64s[6].read(address[:6]), r.ram64s[7].read(address[:6]), address[6:])
}
...
func (r *ram4k) tick(in []bool, address []bool, load bool) {
	a0, a1, a2, a3, a4, a5, a6, a7 := dmux8Way(load, address[9:])
	r.ram512s[0].tick(in, address[:9], a0)
	r.ram512s[1].tick(in, address[:9], a1)
	r.ram512s[2].tick(in, address[:9], a2)
	r.ram512s[3].tick(in, address[:9], a3)
	r.ram512s[4].tick(in, address[:9], a4)
	r.ram512s[5].tick(in, address[:9], a5)
	r.ram512s[6].tick(in, address[:9], a6)
	r.ram512s[7].tick(in, address[:9], a7)
}

func (r *ram4k) read(address []bool) []bool {
	return mux8WayMulti(r.ram512s[0].read(address[:9]), r.ram512s[1].read(address[:9]), r.ram512s[2].read(address[:9]), r.ram512s[3].read(address[:9]), r.ram512s[4].read(address[:9]), r.ram512s[5].read(address[:9]), r.ram512s[6].read(address[:9]), r.ram512s[7].read(address[:9]), address[9:])
}
...
func (r *ram16k) tick(in []bool, address []bool, load bool) {
	a0, a1, a2, a3 := dmux4Way(load, address[12:])
	r.ram4ks[0].tick(in, address[:12], a0)
	r.ram4ks[1].tick(in, address[:12], a1)
	r.ram4ks[2].tick(in, address[:12], a2)
	r.ram4ks[3].tick(in, address[:12], a3)
}

func (r *ram16k) read(address []bool) []bool {
	return mux4WayMulti(r.ram4ks[0].read(address[:12]), r.ram4ks[1].read(address[:12]), r.ram4ks[2].read(address[:12]), r.ram4ks[3].read(address[:12]), address[12:])
}
